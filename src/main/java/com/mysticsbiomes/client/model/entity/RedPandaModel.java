...
import com.mysticsbiomes.client.animation.RedPandaAnimation;
...
public class RedPandaModel<T extends RedPanda> extends HierarchicalModel<T> {
    ...
    public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        this.animate(entity.idleAnimationState, RedPandaAnimation.IDLE, ageInTicks);
        this.animate(entity.sleepingAnimationState, RedPandaAnimation.SLEEP, ageInTicks);

        if (!entity.isSleeping()) {
            if (entity.isSprinting()) {
                this.animateWalk(RedPandaAnimation.SPRINT, limbSwing, limbSwingAmount, 6.5F, 2.5F);
            } else {
                this.animateWalk(RedPandaAnimation.WALK, limbSwing, limbSwingAmount, 10.0F, 8.5F);
            }
        }
    }

    protected void animateWalk(AnimationDefinition definition, float swing, float swingAmount, float speed, float animationSpeed) {
        long i = (long)(swing * 50.0F * speed);
        float f = Math.min(swingAmount * animationSpeed, 1.0F);
        KeyframeAnimations.animate(this, definition, i, f, new Vector3f());
    }
