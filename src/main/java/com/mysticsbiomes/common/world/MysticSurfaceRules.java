...
public class MysticSurfaceRules {
    ...
    private static SurfaceRules.RuleSource mysticRule() {
        SurfaceRules.RuleSource lushSand = SurfaceRules.sequence(SurfaceRules.ifTrue(surfaceNoiseAbove(1.75D), PINK_LUSH_SAND), SurfaceRules.ifTrue(surfaceNoiseAbove(-0.95D), LUSH_SAND));

        SurfaceRules.RuleSource sandstoneLinedSand = SurfaceRules.sequence(SurfaceRules.ifTrue(SurfaceRules.ON_CEILING, RED_SANDSTONE), lushSand);
        return SurfaceRules.ifTrue(SurfaceRules.isBiome(MysticBiomes.LUSH_OASIS), sandstoneLinedSand);
    }
