...
@OnlyIn(Dist.CLIENT)
public class StrawberryBushLayer<T extends StrawberryCowEntity> extends LayerRenderer<T, CowModel<T>> {
    public StrawberryBushLayer(IEntityRenderer<T, CowModel<T>> rendererIn) {
        super(rendererIn);
    }
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (!entitylivingbaseIn.isChild() && !entitylivingbaseIn.isInvisible()) {
            ...
            BlockState blockstate = entitylivingbaseIn.getStrawberryCowType().getRenderState();
